...
const en = `# Local Storage MCP Server
...
> GitHub README does not support native tabs. This doc uses **collapsible sections** (\`<details>\`) to keep the overview scannable ${mdash} click a heading to expand.
...
<details>
<summary><code>remember_fact</code> ${mdash} store a learning</summary>
...
**Checkpoint:** save only when the user explicitly confirms the nuance should be persisted ${mdash} not during exploration.

</details>

<details>
<summary><code>recall_facts</code> ${mdash} free-text search</summary>
...
<details>
<summary><code>recall_by_topic</code> ${mdash} structured search</summary>

Exact topic search ${mdash} less noise than \`recall_facts\`.
...
Automatic migration on startup ${mdash} existing databases gain new columns without data loss.
...
**v1.4.1** ${mdash} \`fact_hash\` backfill \u00b7 full legacy deduplication

**v1.4.0** ${mdash} \`format=compact\` \u00b7 \`recall_by_topic\` \u00b7 access counters \u00b7 checkpoint

**v1.3.0** ${mdash} on-idle consolidator \u00b7 anchor/detail \u00b7 \`fact_hash\`

</details>

---

ISC License ${mdash} see [LICENSE](LICENSE).
`;

const pt = en
  .replace("**Languages:**", "**Idiomas:**")
  .replace("[English](README.md) \u00b7 [Portugu\u00eas (BR)](README.pt-BR.md)",
  ...
  .replace("A [Model Context Protocol (MCP)](https://modelcontextprotocol.io) server for persistent local memory. Lets agents (Cursor, Claude Desktop, etc.) store and retrieve business-rule nuances, architectural decisions, and domain knowledge without bloating the conversation context.",
    "Servidor [Model Context Protocol (MCP)](https://modelcontextprotocol.io) para mem\u00f3ria persistente local. Permite que agentes (Cursor, Claude Desktop, etc.) gravem e recuperem nuances de regras de neg\u00f3cio, decis\u00f5es arquiteturais e conhecimento de dom\u00ednio ${mdash} sem inflar o contexto da conversa.".replace("${mdash}", dash))
  ...
  .replace(`${mdash} store a learning`, `${mdash} gravar aprendizado`)
  ...
  .replace(`${mdash} free-text search`, `${mdash} busca livre`)
  ...
  .replace(`${mdash} structured search`, `${mdash} busca por t\u00f3pico`)
  ...
  .replace("**v1.4.1**", "**v1.4.1**")
  .replace("access counters", "contadores de acesso")
  .replace("checkpoint", "checkpoint")
  .replace("on-idle consolidator", "consolidador on-idle");

fs.writeFileSync(path.join(root, "README.md"), en, "utf8");
fs.writeFileSync(path.join(root, "README.pt-BR.md"), pt, "utf8");
console.log("OK", fs.readFileSync(path.join(root, "README.pt-BR.md"), "utf8").includes("mem\u00f3ria"));
